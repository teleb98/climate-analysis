...
# 모든 관측소 ID 목록
# stnIds_list = [
#     90, 93, 95, 98, 99, 100, 101, 102, 104, 105, 106, 108, 112, 114, 
#     115, 119, 121, 127, 129, 130, 131, 133, 135, 136, 137, 138, 140, 143, 
#     146, 152, 155, 156, 159, 162, 165, 168, 169, 170, 172, 174, 177, 184, 
#     185, 188, 189, 192, 201, 202, 203, 211, 212, 216, 217, 221, 226, 232, 
#     235, 236, 238, 239, 243, 244, 245, 247, 248, 251, 252, 253, 254, 255, 
#     257, 258, 259, 260, 261, 262, 263, 264, 266, 268, 271, 272, 273, 276, 
...
# 2003년부터 2022년까지 50년간의 데이터 수집, 1년 단위로 API 호출하여 저장
# start_year = 2003
# end_year = 2023
def get_temperature_data(api_url, api_key, stnId, start_year, end_year):
    temperature_data = []
            ...
            for day in range(1, 32):
                ...
                if response.status_code == 200:
                    ...
                    for item in root.findall('.//item'):
                        ...
                        if ta is not None and tm is not None and stnNm is not None:
                            temperature_data.append([stnNm.text, tm.text, ta.text])
    return temperature_data

# CSV 파일로 저장할 디렉토리 설정
directory = r'C:\test files\climate analysis\data\temperature raw'
if not os.path.exists(directory):
    os.makedirs(directory)

# 폴더에 file이 있는지 확인하고 있으면 skip 하는 코드 추가
for stnId in stnIds_list:
    for start_year in range(2003, 2023, 1):        
        end_year = start_year + 1
        csv_file_path = os.path.join(directory, f'temperature_data_{stnId}_{end_year}.csv')

        if os.path.exists(csv_file_path):
            print(f"관측소 ID {stnId}의 기온 데이터가 '{csv_file_path}'에 이미 저장되어 있습니다.")
            continue
        else:
            temperature_data = get_temperature_data(api_url, api_key, stnId, start_year, end_year)
            # 관측소별로 CSV 파일로 저장
            with open(csv_file_path, 'w', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f)
                writer.writerow(['Station Name', 'Time', 'temperature'])  # 컬럼 헤더 추가
                writer.writerows(temperature_data)
            print("관측소 ID {stnId}의 기온 데이터가 '{csv_file_path}'에 저장되었습니다.")    